...
class Backwardh extends Component {
  constructor() {
    ...
    this.Backwardh = this.Backwardh.bind(this);
}
Backwardh(d,h,x) {
  fx = this.state.fx;

      if(d===1){
        y = (this.func(x) - this.func(x-(h))) / h
      }
      else if(d===2){
        y = ((this.func(x) - 2*this.func(x-(h)) + this.func(x-(2*h))) / Math.pow(h,2))
      }
      else if(d===3){
        y=((this.func(x) - 3*this.func(x-(h)) + 3*this.func(x-(2*h)) - this.func(x-(3*h))) / Math.pow(h,3))
      }
      else if(d===4){
        y=((this.func(x) - 4*this.func(x-(h)) + 6*this.func(x-(2*h))- 4*this.func(x-(3*h)) + this.func(x-(4*h))) / Math.pow(h,4))
      }
    console.log(y)
  ...
  render() {
    return (
      <Layout style={{ minHeight: '100vh' }}>
                  ...
                  <h2 style={{ color: "black", fontWeight: "bold" }}>Backwardh</h2>
                      ...
                      >
                          ...
                          <Button onClick={() => this.Backwardh(parseFloat(this.state.d),parseFloat(this.state.h),parseFloat(this.state.x))
                          }
